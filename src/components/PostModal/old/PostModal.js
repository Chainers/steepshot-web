...
class PostModal extends React.Component {
  ...
  setCurrentPostOffset(nextProps) {
    ...
    if (
      post &&
      post.offsetTop !== 0 &&
      post.offsetTop !== nextProps.offsetTop
    ) {
  ...
  renderFullScreenImg() {
    ...
    return (
      <div>
        <div
          className="full-image-wrap_pos-mod"
        ...
        >
          ...
          <ShowIf show={this.props.post.isVideo}>
            <div
              className="video-con-fs_pos-mod"
              style={this.props.style.imgCont || previousImageWidth}
            >
              <ReactPlayer
                height="100%"
        ...
        <div>
          <ShowIf show={!this.props.firstPost}>
            <div
              className="arrow-left-full-screen_post-mod"
          ...
          <ShowIf show={!this.props.lastPost && !this.props.newPostsLoading}>
            <div
              className="arrow-right-full-screen_post-mod"
          ...
          <ShowIf show={this.props.newPostsLoading}>
            <div
              className="loader-right-full-screen_post-mod"
              onMouseEnter={this.fsNavMouseEnter.bind(this)}
              onMouseLeave={this.fsNavMouseLeave.bind(this)}
            >
              <LoadingSpinner
        ...
        <div
          className="close-full-screen_pos-mod"
        ...
        >
          <img
            ...
            src="/images/shape-copy-6.svg"
            alt="close full screen"
          />
        </div>
        <div
          className="cross-wrapper_modal"
  ...
  render() {
    ...
    if (this.props.previousStyle)
      previousContainerStyle = this.props.previousStyle.container;
    ...
    return (
      <Fragment>
        <div className="container_pos-mod" style={hideModalFS}>
          ...
          <RenderImage
            previousStyle={this.props.previousStyle}
          ...
          <div className="header_pos-mod" style={this.props.style.headerCont}>
            <div className="date_pos-mod">
              <TimeAgo
                datetime={this.props.post.created}
                locale="en_US"
  ...
  setPostModalSize(isFullScreen = false) {
    let style = setComponentSize(
      this.props.window,
      this.props.post.media[0].size,
      isFullScreen
    );
...
const mapStateToProps = state => {
  ...
  let linkToSinglePost =
    document.location.origin +
    (isGolosService ? "/" + Constants.SERVICES.golos.name : "") +
    "/post" +
    post.url.replace(/\/[\w-.]+/, "");
  let onlyPostModalOpen = Object.keys(state.modals).length === 1;
  if (post) {
    const notFullScreenByScreenSize =
      state.window.width < Constants.WINDOW.MAX_MOBILE_SCREEN_WIDTH;
    ...
    let isCommentEditing = state.comments[currentIndex]
      ? state.comments[currentIndex].commentEditing
      : null;
    return {
      post,
      postsList,
      ...
      focusedTextInput: state.textInput[Constants.TEXT_INPUT_POINT.COMMENT]
        ? state.textInput[Constants.TEXT_INPUT_POINT.COMMENT].focused
        : false,
      window: state.window,
